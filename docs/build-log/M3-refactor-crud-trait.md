# M3 cleanup — the `WithCrudTable` trait (a refactor, explained)

> **What this is, in one sentence:**
> We removed the copy-pasted plumbing shared by the five master-data screens by moving
> it into one reusable **trait** — changing the *structure* of the code without changing
> what it does.
...
A **trait** is a bundle of properties and methods you can **mix into** many classes with a
`use` statement — a way to share code between classes that don't (and shouldn't) inherit
...
Think of it as a reusable "mix-in" of behaviour.
...
> **The Rule of Three:** don't abstract on the 1st or 2nd copy (you don't yet know the real
> pattern). By the **3rd**, extract it. We had **five** — well past the threshold, and the
> repetition was *measured, not guessed*. That's the signal to refactor.

---

## C. What moved, and what stayed
...
## D. Why a trait, not a base class
...
> all five screens to one fragile parent. This is the stronger design here.
...
tables at all — forcing them into the trait would be a bad fit. The trait is for *paginated
master CRUD tables*; those screens are a different family.

---

## E. The safety net (the most important part)
...
**Files:** new `app/Livewire/Concerns/WithCrudTable.php`; the five
`app/Livewire/MasterData/*` components now `use` it. No view, route, or test changes were
needed — nothing the user sees changed.

---

## ✅ Cleanup done. The trait is ready for the next CRUD screen to reuse from day one.
