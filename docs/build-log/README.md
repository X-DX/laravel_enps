...
| M6 — Central Register | [M6 central register](M6-central-register.md) | ✅ all 6 screens (201–206): Entry CR generation (auto + attach-to-existing, locked counter), View All / Pending / Finalized lists, Block CR + Blocked CR List (freeze by CR No with reason, unblock, edit-reason; guarded + owner-scoped) — 226 tests |
| M6 — Central Register numbering | [M6 central register numbering](M6-central-register-numbering.md) | ✅ the three identifiers untangled (First Receipt No / CR No / Receipt No); legacy counter race reproduced and fixed — 146 duplicate CR Nos renumbered with an audit log, `PRIMARY KEY` + 2 indexes + `NOT VALID` FK added to `central_reg`; N+1 removed from inside the counter lock; deterministic `centralReg` relation + `×n` flag for the 249 double-booked drafts; View All / Finalized tables relabelled and completed (CR No column, blocked badge, date range, index-friendly search) — 219 tests |
...
| Migration to UPS (menu 161) | [migrate-to-ups.md](migrate-to-ups.md) | ✅ restored missing feature — data migration adds the menu item + permission; Livewire search → migrate NPS→UPS (guarded, transactional update + `ups_migration` log); **made cross-operator 2026-08-25** — the ownership scope made the search empty and turned the guarded update into a silent no-op — 197 tests |
| Row-level ownership (per-user data) | [row-level-ownership.md](row-level-ownership.md) | ✅ cross-cutting — `OwnedByUser` trait + global scope on `Subscriber`/`FirstReceipt`; per-user lists/detail/export/badges/dashboard, admins see all; route-model binding 404s non-owners; **carve-out: the 3 per-account action screens (Assign PRAN, Close Account, Migrate to UPS) are cross-operator** via `Subscriber::acrossOperators()` — ownership scopes belong on lists you browse, never on relations you resolve or on guarded mass updates — 197 tests |
