    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
        <table class="min-w-full divide-y divide-slate-200 dark:divide-white/10">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                @forelse ($ddos as $ddo)
                    <tr wire:key="ddo-{{ $ddo->ddo_code }}" class="text-sm transition hover:bg-slate-50 dark:hover:bg-white/5">
                        <td class="px-6 py-3 font-medium text-slate-900 dark:text-white">{{ $ddo->ddo_code }}</td>
                        ...
                        <td class="px-6 py-3">
                            <div class="flex items-center justify-end gap-2">
                                <button wire:click="edit({{ $ddo->ddo_code }})" type="button"
                                ...
                                <button type="button"
                                    ...
                                    }).then((result) => { if (result.isConfirmed) { $wire.delete({{ $ddo->ddo_code }}) } })"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-10 text-center text-sm text-slate-400">No DDOs found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
