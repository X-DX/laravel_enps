<div class="mx-auto max-w-4xl">
    ...
    <div class="mb-6 flex items-end justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Locations, each belonging to a district.</p>
        </div>
    ...
    @if ($showForm)
        <form wire:submit="save"
            ...
            <h2 class="mb-4 text-sm font-semibold text-slate-900 dark:text-white">
                {{ $editingCode ? 'Edit location' : 'New location' }}
            </h2>

            <div class="grid gap-4 sm:grid-cols-3">
                <div>
                    <label for="loc_code" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Location code</label>
                    <input wire:model="loc_code" id="loc_code" type="number" @if ($editingCode) readonly @endif
                    ...
                    @error('loc_code') <p class="mt-1.5 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="loc_name" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Location name</label>
                    <input wire:model="loc_name" id="loc_name" type="text"
                    ...
                    @error('loc_name') <p class="mt-1.5 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p> @enderror
                </div>
                <div>
    ...
    {{-- Toolbar: district filter · per-page · Excel --}}
    <div class="mb-4 flex flex-wrap items-center gap-3">
        <div class="flex min-w-56 flex-1 items-center gap-2">
            ...
            <select wire:model.live="filterDistrict" id="filterDistrict"
                class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 transition focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 dark:border-white/10 dark:bg-white/5 dark:text-white">
        ...
        <table class="min-w-full divide-y divide-slate-200 dark:divide-white/10">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                @forelse ($locations as $location)
                    <tr wire:key="loc-{{ $location->loc_code }}" class="text-sm transition hover:bg-slate-50 dark:hover:bg-white/5">
                        <td class="px-6 py-3 font-medium text-slate-900 dark:text-white">{{ $location->loc_code }}</td>
                        <td class="px-6 py-3 text-slate-600 dark:text-slate-300">{{ $location->loc_name }}</td>
                        <td class="px-6 py-3 text-slate-600 dark:text-slate-300">
                            {{ $location->district?->dist_name ?? '—' }}
                            <span class="text-slate-400">({{ $location->dist_code }})</span>
                        </td>
                        <td class="px-6 py-3">
                            <div class="flex items-center justify-end gap-2">
                                <button wire:click="edit({{ $location->loc_code }})" type="button"
                                ...
                                <button type="button"
                                    @click="Swal.fire({
                                        title: 'Delete location {{ $location->loc_code }}?',
                                        text: '{{ addslashes($location->loc_name) }} will be removed. This cannot be undone.',
                                        icon: 'warning',
                                        showCancelButton: true,
                                        confirmButtonText: 'Yes, delete',
                                        confirmButtonColor: '#e11d48',
                                        cancelButtonColor: '#64748b',
                                    }).then((result) => { if (result.isConfirmed) { $wire.delete({{ $location->loc_code }}) } })"
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-10 text-center text-sm text-slate-400">
                            No locations found{{ $filterDistrict ? ' in the selected district' : '' }}.
                        </td>
                    </tr>
                @endforelse
            </tbody>
