<div class="mx-auto max-w-4xl">
    {{-- Header --}}
    <x-breadcrumbs class="mb-4" :crumbs="['Admin Section' => null, 'Master Entry' => null]" current="Designation Master" />

    <div class="mb-6 flex items-end justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight text-slate-900 dark:text-white">Designation Master</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Job designations. The ID is assigned automatically.</p>
        </div>
    ...
    @if ($showForm)
        <form wire:submit="save"
            ...
            <h2 class="mb-4 text-sm font-semibold text-slate-900 dark:text-white">
                {{ $editingId ? 'Edit designation #'.$editingId : 'New designation' }}
            </h2>

            <div>
                <label for="designation" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Designation</label>
                <input wire:model="designation" id="designation" type="text" maxlength="180"
                ...
                @error('designation') <p class="mt-1.5 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p> @enderror
                @unless ($editingId) <p class="mt-1.5 text-xs text-slate-400">A new ID number is assigned automatically on save.</p> @endunless
            </div>
    ...
    <div class="mb-4 flex flex-wrap items-center gap-3">
        <div class="relative min-w-56 flex-1">
            ...
            <input wire:model.live.debounce.300ms="search" type="search" placeholder="Search id or designation…"
        ...
        <table class="min-w-full divide-y divide-slate-200 dark:divide-white/10">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                @forelse ($designations as $designation)
                    <tr wire:key="desig-{{ $designation->designation_id }}" class="text-sm transition hover:bg-slate-50 dark:hover:bg-white/5">
                        <td class="px-6 py-3 font-medium text-slate-900 dark:text-white">{{ $designation->designation_id }}</td>
                        <td class="px-6 py-3 text-slate-600 dark:text-slate-300">{{ $designation->designation }}</td>
                        <td class="px-6 py-3">
                            <div class="flex items-center justify-end gap-2">
                                <button wire:click="edit({{ $designation->designation_id }})" type="button"
                                ...
                                <button type="button"
                                    @click="Swal.fire({
                                        ...
                                        text: '{{ addslashes($designation->designation) }} will be removed. This cannot be undone.',
                                        icon: 'warning',
                                        showCancelButton: true,
                                        confirmButtonText: 'Yes, delete',
                                        confirmButtonColor: '#e11d48',
                                        cancelButtonColor: '#64748b',
                                    }).then((result) => { if (result.isConfirmed) { $wire.delete({{ $designation->designation_id }}) } })"
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-6 py-10 text-center text-sm text-slate-400">
                            No designations found{{ $search ? ' for "'.$search.'"' : '' }}.
                        </td>
                    </tr>
                @endforelse
            </tbody>
