<div class="mx-auto max-w-4xl">
    ...
    <div class="mt-2 mb-5">
        <h1 class="text-2xl font-semibold tracking-tight text-slate-900 dark:text-white">Issue Account</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Enter a new subscriber. Saved as a draft — the account number is generated at finalize.</p>
    </div>

    @php
    // Reusable class strings, so each field below stays short and consistent.
    $input = 'block w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 transition focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400 dark:border-white/10 dark:bg-white/5 dark:text-white dark:disabled:bg-white/5';
        ...
        <div class="{{ $card }}">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input wire:model="father_name" id="father_name" type="text" @disabled($single_mother_flag) class="{{ $input }}">
                    @error('father_name') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <label for="single_mother_flag" class="text-sm text-slate-700 dark:text-slate-300">Single Mother</label>
                </div>
                <div>
                    <label for="dob" class="{{ $label }}">Date of Birth</label>
                    <input wire:model.live="dob" id="dob" type="date" class="{{ $input }}">
                    @error('dob') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
            </div>
        </div>
        ...
        <div class="{{ $card }}">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    @error('appnt_ord_no') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    ...
                    @error('doapptorder') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="doj" class="{{ $label }}">Date of Joining</label>
                    <input wire:model="doj" id="doj" type="date" class="{{ $input }}">
                    @error('doj') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="dor" class="{{ $label }}">Retirement Date <span class="text-xs text-slate-400">(auto-filled)</span></label>
                    <input wire:model="dor" id="dor" type="date" class="{{ $input }}">
                    @error('dor') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="{{ $label }}">Designation</label>
                    <x-searchable-select model="designation" :options="$desigOptions" placeholder="Select designation…" />
                    @error('designation') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="{{ $label }}">Department</label>
                    <x-searchable-select model="nameofdept" :options="$deptOptions" placeholder="Select department…" />
                    @error('nameofdept') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="{{ $label }}">Treasury Location</label>
                    <x-searchable-select model="treasury_code" :options="$treasuryOptions" placeholder="Select treasury…" :live="true" />
                    @error('treasury_code') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="{{ $label }}">DDO</label>
                    <x-searchable-select model="ddocode" :options="$ddoOptions"
                        :placeholder="$treasury_code === '' ? 'Select a treasury first' : 'Select DDO…'"
                        :disabled="$treasury_code === ''"
                ...
                <div>
                    ...
                    <div class="flex items-center gap-6 pt-1">
                        <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                            <input wire:model.live="pension_type" type="radio" value="N" class="text-indigo-600 focus:ring-indigo-500/30"> NPS
                        </label>
                        <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                            <input wire:model.live="pension_type" type="radio" value="U" class="text-indigo-600 focus:ring-indigo-500/30"> UPS
                        </label>
                    </div>
                    @error('pension_type') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="pay" class="{{ $label }}">{{ $pension_type === 'N' ? 'Basic + DA' : 'Basic Pay' }}</label>
                    <input wire:model="pay" id="pay" type="text" inputmode="numeric" class="{{ $input }}">
                    @error('pay') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    ...
                    @error('starting_month') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    ...
                    @error('starting_fin_year') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
            </div>
        </div>
        ...
        <div class="{{ $card }}">
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                <div>
                    ...
                    @error('name_nominee') <p class="{{ $err }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="name_nominee2" class="{{ $label }}">2nd Nominee</label>
                    <input wire:model="name_nominee2" id="name_nominee2" type="text" class="{{ $input }}">
                </div>
                <div>
                    <label for="name_nominee3" class="{{ $label }}">3rd Nominee</label>
                    <input wire:model="name_nominee3" id="name_nominee3" type="text" class="{{ $input }}">
                </div>
            </div>
        </div>
        ...
        <div class="{{ $card }}">
            <div class="flex flex-wrap items-end justify-between gap-4">
                <div>
                    ...
                    <p class="rounded-xl bg-slate-100 px-3 py-2.5 text-sm font-semibold text-slate-500 dark:bg-white/5 dark:text-slate-400">PENDING</p>
                    <p class="mt-1 text-xs text-slate-400">Generated automatically on finalize.</p>
                </div>
