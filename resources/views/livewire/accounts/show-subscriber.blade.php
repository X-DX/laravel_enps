<div class="mx-auto max-w-4xl">
    ...
    <div class="mt-3 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
        <div class="flex flex-wrap items-center gap-4 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-sky-50 p-6 dark:border-white/5 dark:from-indigo-500/10 dark:to-sky-500/10">
            <div class="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-sky-500 text-lg font-bold text-white shadow-md shadow-indigo-500/30">
                {{ $initials ?: '?' }}
            </div>
            <div class="min-w-0 flex-1">
                <h1 class="truncate text-xl font-semibold text-slate-900 dark:text-white">{{ $subscriber->name }}</h1>
                <p class="mt-0.5 text-sm text-slate-500 dark:text-slate-400">{{ $subscriber->designationMaster?->designation ?? '—' }}</p>
            </div>
            @if ($subscriber->save_flag === 'F')
                <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300">
        ...
        <div class="grid grid-cols-2 gap-px bg-slate-100 sm:grid-cols-3 dark:bg-white/5">
            @foreach (['Account No' => $accountNo, 'PRAN' => $pran, 'DDO' => $subscriber->ddo?->ddo_name ?? '—'] as $label => $value)
                <div class="bg-white p-4 dark:bg-slate-900">
                    ...
                    <p class="mt-0.5 truncate font-semibold text-slate-900 dark:text-white">{{ $value }}</p>
                </div>
            @endforeach
        </div>
    </div>
                ...
                <dl class="grid grid-cols-2 gap-x-4 gap-y-3">
                    @foreach ($fields as $label => $value)
                        <div>
                            ...
                            <dd class="mt-0.5 text-sm font-medium text-slate-800 dark:text-slate-200">{{ filled($value) ? $value : '—' }}</dd>
                        </div>
                    @endforeach
                </dl>
            </div>
