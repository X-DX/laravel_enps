<div class="mx-auto max-w-3xl">
    ...
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Move a finalized NPS account to the Unified Pension Scheme. This can't be undone.</p>
    </div>
    ...
    <div class="{{ $card }}">
        ...
        @if ($results->isNotEmpty())
            <ul class="mt-3 divide-y divide-slate-100 overflow-hidden rounded-xl border border-slate-200 dark:divide-white/5 dark:border-white/10">
                    ...
                    <li>
                        <button wire:click="selectAccount('{{ $r->account_no }}')" type="button"
                            ...
                            <span class="flex items-center gap-2 text-slate-500 dark:text-slate-400">
                                {{ $r->name }}
                                <span class="rounded-md px-1.5 py-0.5 text-[10px] font-bold {{ $r->pension_type === 'U' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300' : 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300' }}">{{ $r->pension_type === 'U' ? 'UPS' : 'NPS' }}</span>
                            </span>
                        </button>
                    </li>
                @endforeach
    ...
    @if ($account)
        <div class="mt-6 {{ $card }}">
            <div class="mb-4 flex items-center justify-between">
                ...
                <button wire:click="closeForm" type="button" class="text-sm text-slate-400 transition hover:text-slate-600 dark:hover:text-slate-200">✕ Close</button>
            </div>
            ...
            <div class="mb-5 grid grid-cols-1 gap-4 rounded-xl bg-slate-50 p-4 sm:grid-cols-2 dark:bg-white/5">
                ...
                <div>
                    ...
                    <p class="mt-0.5">
                        <span class="rounded-md px-2 py-0.5 text-xs font-bold {{ $account->pension_type === 'U' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300' : 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300' }}">
            ...
            @if ($account->pension_type === 'U')
                <div class="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
            ...
            @else
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <div>
                        ...
                        @error('migrationYear') <p class="mt-1.5 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="{{ $label }}">Migration Month</label>
                        <select wire:model="migrationMonth" class="{{ $input }}" @disabled($migrationYear === '')>
                            <option value="">{{ $migrationYear === '' ? 'Select a year first' : 'Select month…' }}</option>
                            @for ($m = 1; $m <= $maxMonth; $m++)
                                <option value="{{ $m }}">{{ $months[$m] }}</option>
                            @endfor
                        </select>
                        @error('migrationMonth') <p class="mt-1.5 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p> @enderror
                    </div>
                </div>
                ...
                <div class="mt-6 flex justify-end">
                    <button wire:click="migrate" type="button" @disabled($migrationYear === '' || $migrationMonth === '')
                        ...
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" /></svg>
                        Migrate to UPS
                    </button>
                </div>
            @endif
