<div class="mx-auto max-w-4xl">
    @php
        ...
        $bankOptions = $banks->map(
            fn($b) => ['value' => $b->bank_code, 'label' => trim($b->bank_name) . ' — ' . trim($b->branch_name)],
        );
    ...
    <div class="mb-5">
        <h1 class="font-display text-2xl font-bold tracking-tight text-slate-900 dark:text-white">
            {{ $editingId ? 'Edit First Entry' : 'Entry First Register' }}</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
            {{ $editingId ? 'Update this pending entry.' : 'Record an incoming receipt / draft. Saved as pending — finalize it later.' }}
        </p>
    </div>
    ...
    <form wire:submit="save" class="space-y-5">
        ...
        <div class="{{ $card }}">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <x-searchable-select model="treasuryCode" :options="$treasuryOptions" placeholder="Select treasury…"
                        :live="true" />
                    @error('treasuryCode')
                ...
                <div>
                    ...
                    <x-searchable-select model="ddocode" :options="$ddoOptions" :placeholder="$treasuryCode === '' && ! $editingId ? 'Select a treasury first' : 'Select DDO…'" :disabled="! $editingId && $treasuryCode === ''"
                        wire:key="ddo-{{ $treasuryCode }}" />
                    @error('ddocode')
                        <p class="{{ $err }}">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="{{ $card }}">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2 flex items-center gap-2">
                    <input wire:model.live="isDraft" id="isDraft" type="checkbox"
                        class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500/30">
                    <label for="isDraft" class="text-sm text-slate-700 dark:text-slate-300">Select in case <span
                            class="font-semibold">Contribution</span> is recevied through <span
                            class="font-semibold">Draft</span></label>
                </div>
                <div>
                    <label for="draftNo"
                        class="{{ $label }}">{{ $isDraft ? 'Draft No' : 'Receipt No' }}</label>
                    <input wire:model="draftNo" id="draftNo" type="text" inputmode="numeric"
                        class="{{ $input }}">
                    @error('draftNo')
                        <p class="{{ $err }}">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    <label for="draftDate"
                        class="{{ $label }}">{{ $isDraft ? 'Draft Date' : 'Receipt Date' }}</label>
                ...
                {{-- Draw Bank only applies to a draft (a draft is drawn on a bank). --}}
                @if ($isDraft)
        ...
        <div class="{{ $card }}">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input wire:model="amount" id="amount" type="text" inputmode="numeric"
                        class="{{ $input }}">
                    @error('amount')
                        <p class="{{ $err }}">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="flex items-center gap-6 pt-1">
                        <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                            <input wire:model="pensionType" type="radio" value="N"
                                class="text-indigo-600 focus:ring-indigo-500/30"> NPS
                        </label>
                        <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                            <input wire:model="pensionType" type="radio" value="U"
                ...
                {{-- Free-text description, only when "OTHERS" is chosen. --}}
                @if ($purpose === 'OTH')
                    <div>
                        <label for="otherPurpose" class="{{ $label }}">Others</label>
                        <input wire:model="otherPurpose" id="otherPurpose" type="text" maxlength="150"
                            placeholder="Describe the purpose…" class="{{ $input }}">
                        @error('otherPurpose')
                            <p class="{{ $err }}">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        <div class="{{ $card }} flex flex-wrap items-center justify-end gap-3">
            @if ($showForceSave)
                <p class="mr-auto text-sm text-amber-600 dark:text-amber-400">A matching draft/receipt already exists.
                </p>
                <button type="button" wire:click="forceSave"
                    wire:confirm="Save anyway? A receipt with this number and date already exists."
